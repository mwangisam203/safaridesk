...
def test_basic_subscriber_can_read_basic_article_without_free_read_tracking(monkeypatch):
    ...
    fake_db = FakeDb(articles=[article], users=[user], subscriptions=[subscription])
...
def test_lapsed_subscriber_loses_paid_access_even_if_tier_column_is_stale(monkeypatch):
    """
    User.subscription_tier is only corrected by the daily lifecycle Celery
    task. If that worker hasn't run since a subscription's grace period
    genuinely ended, the column can still read PRO/BASIC — content access
    must fall back to FREE rules anyway, based on the real Subscription row.
    """
    now = datetime.now(timezone.utc)
    article = make_article(tier=ArticleTier.BASIC)
    user = make_user(tier=SubscriptionTier.PRO)
    lapsed_subscription = Subscription(
        id=9,
        user_id=user.id,
        tier=SubscriptionTierInfo.PRO,
        status=SubscriptionStatus.GRACE_PERIOD,
        started_at=now - timedelta(days=40),
        expires_at=now - timedelta(days=10),  # well past the 3-day grace window
    )
    fake_db = FakeDb(articles=[article], users=[user], subscriptions=[lapsed_subscription])
    ...
    monkeypatch.setattr(content, "already_read", lambda user_id, article_id, db: False)
    monkeypatch.setattr(
        content, "get_free_reads_count", lambda user_id, db: content.FREE_ARTICLE_LIMIT
    )
    ...
    assert detail["action"] == "subscribe"
    assert article.view_count == 0
    # Opportunistically self-healed instead of waiting for the next lifecycle run.
